            ...
            for j in range(tag_num):
                for k in range(tag_num):
                    ...
                    self.tag_trigram_prob[i][j][k] = l1 * (self.tag_uni_count[i] / self.total_tag) + \
                                                    ...
                                                    (1 - l2 - l1) * (self.tag_trigram_count[j][k][i] /( 1+self.tag_bigram_count[k][i]))
...
def main():
#run e. g. python HMM.py --f1 train_data.txt --f2 test_data.txt --l1 0.3 --l2 0.3
    parser = OptionParser()
    ...
    parser.add_option("--l1", dest = "l1", type="float", help = " lambda1 value", default = 0.3 )
    parser.add_option("--l2", dest = "l2", type="float", help = "lambda2 value", default = 0.3 )
    (options, args) = parser.parse_args()
    train = HMM(options.file1, options.file2, 14,options.l1,options.l2)
